...
$users = get-aduser -filter {(Enabled -eq $true) -and (PasswordNeverExpires -eq $false)} -properties Name, givenName, PasswordNeverExpires, PasswordExpired, PasswordLastSet, EmailAddress | where { $_.passwordexpired -eq $false }
# Count Users
...
foreach ($user in $users)
{
    ...
    $userObj | Add-Member -Type NoteProperty -Name givenName -Value $firstname
...
foreach ($user in $notifyUsers)
{
    ...
    <p>To change your password on your Andersen issued computer, press CTRL+ALT+Delete and choose Change Password.
    Please ensure you are connected to the Andersen network when you change your password. If you are a remote user, you will need to connect the VPN prior to changing your password.
    If you are using a MAC computer, you can now change your password via <a href=""http://myit.andersencorp.com/"">My IT</a>. <br>
    <p> Don't forget to update the password on your mobile devices if you connect to Andersen's Mobile Wireless! Failure to update the password on your mobile device will result in account lockouts.
    </P>
    <p>This email is an automated notification, which is unable to receive replies. If you have questions, please contact the IT Service Desk (1-800-579-2112). <br>
    </P>
    <p>Thank you, <br> 
    The IT Collaboration Team <br>
    </font>"
