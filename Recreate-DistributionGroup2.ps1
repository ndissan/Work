...
#              initial export of group attributes, the on-premises group will be moved to an OU      #
#              which will stop it from be synchronized.                                              #
...
<#
	...
	.PARAMETER Finalize
        ...
        .EXAMPLE #2
        	.\Recreate-DistributionGroup.ps1 -Group "DL-Marketing" -FinalizeMembers
...
If ($CreatePlaceHolder.IsPresent) {
    ...
    If (((Get-DistributionGroup $Group -ErrorAction 'SilentlyContinue').IsValid) -eq $true) {
        ...
        #$OldDGGUID = Get-ADGroup $Group
        #$OldNickname = Get-ADGroup $Group -properties *
        #$OldMailAddress = $OldNickname.mailnickname + "@andersencorp.mail.onmicrosoft.com"
        ...
        #$description = "Do Not Use - This group has been migrated to Office 365 " + $date 
        ...
        #Move-ADObject $OldDGGUID.ObjectGUID -TargetPath "OU=Migrated_Groups_Dec_2019,OU=Contacts for O365 Groups,OU=Security Groups,OU=Groups,OU=IT,OU=Corporate,DC=andersencorp,DC=com"
        #set-adgroup $OldName -Description $description 
          
#       New-MailContact -Name $OldDisplayName -ExternalEmailAddress $OldMailAddress
        
        
        #New-ADObject -name $OldDisplayName -Type Contact -path "OU=Contacts_For_Groups_in_O365,OU=Contacts for O365 Groups,OU=Security Groups,OU=Groups,OU=IT,OU=Corporate,DC=andersencorp,DC=com" -OtherAttributes @{
        #    'mail'=$OldMailAddress;
        #    'proxyAddresses'=$OldPrimarySmtpAddress;
        #    'displayname'=$OldDisplayName
        #    }

<#
        Get-ADGroup -identity $OldName | Set-ADGroup -Add @{adminDescription="Group_NoSync"}
        Get-ADObject -filter {adminDescription -eq "Group_NoSync"}
...
ElseIf ($FinalizeMembership.IsPresent) {
  
    $TempDG = Get-DistributionGroup $group
    $TempMembers = (Get-DistributionGroupMember $group).Name
    $TempDGCloud = "Cloud-" + $TempDG.Name
    
    foreach ($TempMember in $TempMembers) {
        Add-DistributionGroupMember `
        -Identity $TempDGCloud `
        -Member "Cloud-$TempMember" `
        -BypassSecurityGroupManagerCheck
    }                
